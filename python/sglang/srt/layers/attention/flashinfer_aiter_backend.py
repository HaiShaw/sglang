...
class FlashInferAiterAttnBackend(AttentionBackend):
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch):
        if forward_batch.forward_mode.is_decode_or_idle():
            ...
            kv_indptr = self.kv_indptr # points to buffer, since self.kv_indices supports 2 wrappers scenario, for aiter decode use kv_indptr[0] as kv_indptr buffer
    ...
    def init_forward_metadata_capture_cuda_graph(
        ...
        if forward_mode.is_decode_or_idle():
            if spec_info is None:
                ...
                kv_indptr, kv_indices = spec_info.kv_indptr, spec_info.kv_indices
            self.forward_metadata = DecodeMetadata(kv_indptr, kv_indices)
            self.decode_cuda_graph_metadata[bs] = self.forward_metadata # cuda_graph_metadata never used?
